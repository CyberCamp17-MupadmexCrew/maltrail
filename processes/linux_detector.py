...
_DATE_INPUT_FORMAT = '%Y/%m/%d %H:%M:%S'  # Date format used by MalTrail
_DATE_FORMAT = '%m/%d/%Y %H:%M:%S'  # Date format used by audit


def search_process(ip_dest, port_dest, timestamp):
    timestamp_start = _calculate_start_timestamp(timestamp)
    raw_last_connection = _get_last_connection(ip_dest, port_dest, timestamp, timestamp_start)
...
def _calculate_start_timestamp(timestamp):
    date = datetime.strptime(timestamp, _DATE_FORMAT)
    start_datetime = date - timedelta(seconds=_PERIOD_OF_SEARCH)

    return start_datetime.strftime(_DATE_FORMAT)
