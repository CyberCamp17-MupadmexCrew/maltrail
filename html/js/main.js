...
var LOG_COLUMNS = { TIME: 0, SENSOR: 1, SRC_IP: 2, SRC_PORT: 3, DST_IP: 4, DST_PORT: 5, PROTO: 6, TYPE: 7, TRAIL: 8, INFO: 9, REFERENCE: 10, PID: 11, PROCESS_NAME: 12 };
var LOG_COLUMNS_SIZE = 0;
var DATATABLES_COLUMNS = { THREAT: 0, SENSOR: 1, EVENTS: 2, SEVERITY: 3, FIRST_SEEN: 4, LAST_SEEN: 5, SPARKLINE: 6, SRC_IP: 7, SRC_PORT: 8, DST_IP: 9, DST_PORT: 10, PROTO: 11, TYPE: 12, TRAIL: 13, INFO: 14, REFERENCE: 15, TAGS: 16, PID: 17, PROCESS_NAME: 18, KILL_BUTTON: 19 };
    ...
    Papa.parse(demo ? csv : url, {
        ...
        chunk: function(results) {
            ...
            for (var i = 0; i < results.data.length; i++) {
                ...
                threat_text += THREAT_PID_INFIX + data[LOG_COLUMNS.PID]
...
function initDetails() {
    var details = $('#details').dataTable( {
        ...
        columns: [
            ...
            // Columns to show PID and process name.
            { "title": "pid", "class": "center" },
            { "title": "process_name", "class": "center" },
            { "title": "kill_process", "class": "center"}
        ],
        search: {
...
//             },
            ...
            },
            {
                ...
                targets: [ DATATABLES_COLUMNS.SENSOR, DATATABLES_COLUMNS.PROTO, DATATABLES_COLUMNS.PID, DATATABLES_COLUMNS.PROCESS_NAME ]
            },
            {
                render: function (data, type, row) {

                    var ipSrc = row[DATATABLES_COLUMNS.SRC_IP];
                    var pid = row[DATATABLES_COLUMNS.PID];
                    
                    if(pid === '-')
                        return '-'
                    
                    return "<button data-ip-src='"+ipSrc+"' data-pid='"+pid+"' class='kill-button'>Kill</button>"
                },
                targets: [ DATATABLES_COLUMNS.KILL_BUTTON ]
            },
            {
                render: function (data, type, row) {
...
function hookupKillButtons(){
    $('.kill-button').on('click', killProcess);
}


function killProcess(){
    var pid = $(this).data('pid');
    var ipSrc = $(this).data('ip-src');
